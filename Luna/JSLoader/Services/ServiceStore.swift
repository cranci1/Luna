...
public final class ServiceStore {
    ...
    private func migrateLegacyStoreIfNeeded(preferred: StoreBaseDirectory, targetURL: URL) {
        ...
        for (src, dst) in zip(legacySidecars, targetSidecars) {
            ...
            do {
                ...
                Logger.shared.log("Failed to migrate legacy service store file \(src.lastPathComponent): \(error.localizedDescription)", type: "CloudKit")
            }
        }
    }
    ...
    private func switchToProfile(_ profileID: String) {
        ...
        Logger.shared.log("Switching ServiceStore to profile: \(profileID)", type: "CloudKit")
